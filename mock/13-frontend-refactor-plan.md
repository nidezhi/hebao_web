...
详细到接口、字典、DTO、可视化组件、页面生成步骤和验收清单的落地蓝图见：

```text
docs_new/18-frontend-generation-blueprint.md
```
...
- 所有用户选择后需要回传的 `bizId`、`*BizId`、`businessBizId`、`targetBizId`、`userBizId` 等字段必须使用对象选择器或可搜索下拉；界面展示名称、编码、状态、时间等可识别信息，提交值才是真实 id。禁止让用户手填裸 BizId，只读证据和高级调试模式除外。
