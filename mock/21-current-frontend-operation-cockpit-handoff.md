...
但 `bizId` 只能作为前端内部值、API 提交值和只读证据字段；凡是需要用户选择后回传的 `bizId`、`*BizId`、`businessBizId`、`targetBizId`、`userBizId` 等字段，必须落成对象选择器或可搜索下拉。控件展示名称、编码、状态、时间等业务可识别信息，实际提交值才是对应 id。除只读证据、禁用上下文字段或明确高级调试模式外，禁止让用户手工输入裸 BizId。

如果后端接口无法支撑上述前端铁律，例如缺少可分页列表、搜索参数、详情摘要、display 字段、状态字典、关联对象信息或错误上下文，前端不得通过手填 id、假数据、硬编码映射或堆原始 JSON 绕过。交接文档必须列出后端补齐清单，逐项说明缺口接口、缺口字段、影响页面、用户体验风险和阻塞的铁律。
